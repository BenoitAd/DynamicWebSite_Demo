-- Create the database if it doesn't already exist (PL/pgSQL block)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_database WHERE datname = 'ecommerce_demo') THEN
        CREATE DATABASE ecommerce_demo;
END IF;
END $$;

-- Connect to the 'ecommerce_demo' database
\c ecommerce_demo;
